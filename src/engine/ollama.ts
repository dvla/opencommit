...
export class OllamaAi implements AiEngine {
  ...
  async generateSingleCommitMessage(
    messages: string
  ): Promise<string | undefined> {
    const model = 'mistral'; // todo: allow other models

    let prompt = messages;
    prompt += 'Summarise the following commit messages into a single commit message title ensuring the title format stays the same.';
